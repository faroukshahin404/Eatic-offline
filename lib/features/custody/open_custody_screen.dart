...
/// Open custody confirmation content: "Do you want to open a new custody?" with No / Yes.
/// Uses [BlocConsumer] to show success and close on [CustodyCreateSuccess].
class OpenCustodyScreen extends StatelessWidget {
  const OpenCustodyScreen({super.key});
  ...
  Widget build(BuildContext context) {
    return BlocProvider<CustodyCubit>(
      ...
      child: BlocConsumer<CustodyCubit, CustodyState>(
        listenWhen: (prev, next) => next is CustodyCreateSuccess,
        listener: (context, state) {
          if (state is CustodyCreateSuccess) {
            context.read<CustodyCubit>().restoreAfterCreate();
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(context.tr('custody.created')),
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            context.tr('custody.open_new_confirm'),
  ...
  Future<void> _onYesPressed(BuildContext context) async {
    ...
    await context.read<CustodyCubit>().addNew(totalWhenCreate: amount);
  }
}
